...
export default function Food() {
  ...
  useEffect(() => {
    ...
    fetchItems();
    // Load food reviews
    setReviews(mockReviews.filter(review => review.item_type === 'food'));
  }, []);
  ...
  const fetchItems = async () => {
    try {
      const res = await fetch('/.netlify/functions/food');
      if (res.ok) {
        const data = await res.json();
        setItems(data);
      }
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            ...
            <nav className="hidden md:flex space-x-8">
              ...
              <Link to="/food" className="text-orange-600 font-semibold">Food</Link>
      ...
      <section className="bg-gradient-to-r from-orange-500 to-red-600 text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold mb-4">Culinary Delights</h2>
          <p className="text-xl mb-8">Taste authentic foods from nations around the world</p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          <button
            ...
            className={`p-4 rounded-lg border-2 transition-colors ${
              selectedNation === null ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300'
            }`}
          >
          ...
          {nations.map((nation) => (
            <button
              ...
              className={`p-4 rounded-lg border-2 transition-colors ${
                selectedNation === nation.id ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300'
              }`}
            >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-bold">Food Items</h3>
        ...
        {getFilteredItems().length === 0 && (
          <p className="text-center text-gray-500 mt-8">No food items found.</p>
        )}
      </section>
